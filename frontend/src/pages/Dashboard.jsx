...
const StatsBar = ({ resumes }) => {
    ...
    return (
        <div className="grid grid-cols-3 gap-4 mb-6">
            {[
                { label: 'Total Resumes', value: resumes.length, icon: '📄', color: 'from-violet-500 to-fuchsia-500' },
                { label: 'Avg Completion', value: `${avgCompletion}%`, icon: '📊', color: 'from-blue-500 to-cyan-500' },
                { label: 'Last Edited', value: lastEdited, icon: '🕒', color: 'from-emerald-500 to-teal-500' },
            ].map((stat) => (
                <div key={stat.label} className="bg-white border border-gray-100 rounded-2xl p-4 flex items-center gap-4 shadow-sm">
...
const Dashboard = () => {
    ...
    const calculateCompletion = (resume) => {
        ...
        resume.skills?.forEach(s => {
            total += 2
            if (s.name) done++; if (s.progress > 0) done++
        })
        ...
        resume.certifications?.forEach(c => {
            total += 3
            if (c.title) done++; if (c.issuer) done++; if (c.year) done++
        })
        resume.languages?.forEach(l => {
            total += 2
            if (l.name) done++; if (l.progress > 0) done++
        })
    ...
    const fetchAllResumes = async () => {
        try {
            ...
            const resumesWithCompletion = response.data.map(r => ({
                ...r, completion: calculateCompletion(r)
            }))
            ...
            <Modal isOpen={openCreateModal} onClose={() => setOpenCreateModal(false)} hideHeader>
                <div className="p-6">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <button onClick={() => setOpenCreateModal(false)} className="text-gray-500 hover:text-gray-700">✕</button>
                    </div>
