...
// Add a check to ensure html2canvas is defined before using it
if (typeof html2canvas === 'undefined') {
  console.error('html2canvas is not loaded or defined')
}
...
const EditResume = () => {
  ...
  useEffect(() => {
    ...
    switch (currentPage) {
      ...
      case "additionalInfo":
        if (resumeData.languages.length === 0 || !resumeData.languages[0]?.name || !resumeData.languages[0].name.trim()) {
          errors.push("At least one language is required")
        }
        if (resumeData.interests.length === 0 || !resumeData.interests[0] || !resumeData.interests[0].trim()) {
...
//Take back to previous state or take back to dashboard based on the current page
  const goBack = () => {
    const pages = [
      "profile-info",
      "contact-info",
...
//update array items in the resume state
  ...
  //remove the info using the index
  ...
  //fetch resume details by backend url
...
//It will help in choosing the review as well as helps in downloading the resume also saves the resume as a image
const uploadResumeImages = async (retryCount = 0) => {
    const maxRetries = 3
    ...
    try {
        ...
        const thumbnailCanvas = await html2canvas(fixedThumbnail, {
            scale: 0.3, // Reduced scale for faster processing
        ...
        // Remove the cloned element from DOM after html2canvas is done
        document.body.removeChild(fixedThumbnail)

        //save the image as resume
      ...
      navigate("/dashboard")
        ...
        if (isNetworkError && retryCount < maxRetries) {
            ...
            toast.error(`Upload failed, retrying... (${retryCount + 1}/${maxRetries})`)

            // Wait before retrying
            setTimeout(() => {
                uploadResumeImages(retryCount + 1)
            }, retryDelay)
            return
        }
 ...
 const updateResumeDetails = async (thumbnailLink) => {
    try {
      ...
      await axiosInstance.put(API_PATHS.RESUME.UPDATE(resumeId), {
        ...
        timeout: 30000, // 30 second timeout for large resume data
      })
    } catch (err) {
...
return (
    <DashboardLayout>
        ...
        <Modal isOpen={openThemeSelector} onClose={() => setOpenThemeSelector(false)} title="Change Title">
          ...
          <Modal isOpen={openPreviewModal} onClose={() => setOpenPreviewModal(false)} title={resumeData.title} showActionBtn 
