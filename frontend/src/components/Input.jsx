...
export const Input = ({ value, onChange, label, placeholder, type = 'text', min, max, error, ...rest }) => {
...
// ProfilePhotoSelector and TitleInput stay exactly the same as before — no changes needed
    ...
    return (
        <div className="flex justify-center mb-8">
            ...
            {!previewUrl ? (
                ...
                >
                    <button type="button"
                        className="absolute -bottom-2 -right-2 w-12 h-12 flex items-center justify-center bg-gradient-to-r from-violet-500 to-fuchsia-500 hover:from-violet-600 hover:to-fuchsia-600 text-white rounded-full transition-all shadow-lg hover:scale-110">
                        <Camera size={20} />
                    </button>
                </div>
            ) : (
                <div className="relative group" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
                    ...
                    <div className="absolute inset-0 bg-black/30 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-2">
                        <button type="button" className="w-10 h-10 flex items-center justify-center bg-white/80 text-gray-800 rounded-full hover:bg-white transition-all" onClick={chooseFile}>
                            <Edit size={16} />
                        </button>
                        <button type="button" className="w-10 h-10 flex items-center justify-center bg-red-500 text-white rounded-full hover:bg-red-600 transition-all" onClick={handleRemove}>
                            <Trash2 size={16} />
                        </button>
                    </div>
                </div>
